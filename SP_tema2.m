...
x_tr = sawtooth((2*pi/D)*t_tr,0.5)/2+0.5; %Semnalul triunghiular
...
for i = 1:length(x_tr)
    ...
    if x_tr(i) <= D
        ...
        x_mod(j) = x_tr(i); %modificam valorile nule cu valori din semnalul original
...
plot(t_mod,x_mod),title('x(t)(linie solida) si semnalul reconstruit(linie punctata)'); %afisare x(t)
...
plot(t_mod,x_finit,'--'); %afisare reconstructie semnal folosind cei N coeficienti
